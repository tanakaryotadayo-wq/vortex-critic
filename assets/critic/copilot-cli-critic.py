#!/usr/bin/env python3
"""
copilot-cli-critic.py - ACP/PCC GitHub Copilot CLI critic hook.

This lane shares the VORTEX evidence collector with vortex-critic.py and sends
the same read-only audit packet to GitHub Copilot CLI in plan mode.
"""
...
def load_config() -> dict[str, Any]:
    ...
    config.update({
        ...
        "model": DEFAULT_MODEL,
        "copilotBinary": os.environ.get("VORTEX_COPILOT_BIN", "copilot"),
        "copilotTimeoutSeconds": DEFAULT_TIMEOUT_SECONDS,
        "copilotAddDir": os.environ.get("VORTEX_COPILOT_ADD_DIR", "0") == "1",
        "copilotDisableBuiltinMcps": True,
        "ft_data_dir": os.path.expanduser("~/.vortex-critic/copilot-cli"),
    })

    config_path = Path(__file__).with_name("copilot-cli-critic.config.json")
    if config_path.exists():
        try:
            ...
            print(f"Copilot CLI critic config JSON is invalid: {exc}", file=sys.stderr)
...
def resolve_copilot_binary(config: dict[str, Any]) -> str | None:
    configured = str(config.get("copilotBinary") or "").strip()
    candidates = [
        configured,
...
def run_copilot_cli(prompt: str, config: dict[str, Any], workspace_root: str | None) -> dict[str, Any]:
    copilot_bin = resolve_copilot_binary(config)
    if not copilot_bin:
        return {"text": None, "error": "copilot CLI not found", "exit_code": 127}
    ...
    if config.get("copilotDisableBuiltinMcps", True):
        command.append("--disable-builtin-mcps")
    if config.get("copilotAddDir", False) and workspace_root and Path(workspace_root).is_dir():
        command.extend(["--add-dir", workspace_root])
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=int(config.get("copilotTimeoutSeconds", DEFAULT_TIMEOUT_SECONDS)),
            check=False,
        )
...
def main() -> int:
    ...
    if dry_run:
        ...
        return emit("[Copilot CLI Critic Dry Run | no Copilot call]\n" + json.dumps(dry_payload, ensure_ascii=False, indent=2))

    result = run_copilot_cli(cli_prompt, config, workspace_root if isinstance(workspace_root, str) else None)
    if result.get("text") is None:
        print(f"Copilot CLI critic error: {result.get('error', 'no response')}", file=sys.stderr)
        return emit(None)
    ...
    try:
        ...
        print(f"Copilot CLI FT save failed: {exc}", file=sys.stderr)

    additional_context = (
        f"[Copilot CLI Critic | model:{result.get('model')} | PCC Stack:{stack_label} | "
